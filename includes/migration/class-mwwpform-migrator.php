...
class FPLANT_MWWPForm_Migrator extends FPLANT_Migrator_Base {
	...
	public function migrate( $source_id ) {
		...
		// R2: Button shortcodes are transferred to settings in build_form_settings(), so no skip warning is emitted here.
		...
		foreach ( $parsed['skipped'] as $skipped ) {
			if ( in_array( $skipped['shortcode'], $button_shortcodes, true ) ) {
				continue;
			}
			$this->add_warning(
				self::LEVEL_INFO,
				...
				array( 'shortcode' => $skipped['shortcode'] )
			);
		}
	...
	private function build_form_settings( array $buttons, array $mw_meta, $use_confirmation, array $templates ) {
		...
		// During MW WP Form migration, set the default form design CSS to "none".
		// (Respects the existing markup and styles from MW WP Form; Form Plant decorative CSS is not applied.)
		$settings['design_type'] = 'none';
		$this->add_warning(
			self::LEVEL_INFO,
			'form_design_set_none',
			__( 'The migrated form\'s form design has been set to "none" (Form Plant decorative CSS will not be applied). If you want to retain the original MW WP Form appearance, adjust the CSS in the layout settings as needed.', 'form-plant' )
		);
		...
		// R3: Enable the input screen HTML template
		if ( isset( $templates['input_template'] ) && '' !== $templates['input_template'] ) {
			$settings['use_html_template'] = true;
		}

		// R3: Confirmation screen HTML template
		if ( $use_confirmation && isset( $templates['confirmation_template'] ) && '' !== $templates['confirmation_template'] ) {
			$settings['use_confirmation_template'] = true;
	...
	/**
	 * Generates the new form title in the format "<original title>(from WP MW Form YYYY-MM-DD HH:MM:SS)".
	 *
	...
	private function build_new_form_title( $source_title, $timestamp = null ) {
		...
		return sprintf(
			...
			__( '%1$s(from WP MW Form %2$s)', 'form-plant' ),
			$base_title,
			$timestamp
		);
	}
